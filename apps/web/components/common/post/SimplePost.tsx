...
export function SimplePost({ title, createdBy, linkUrl }: SimplePostProps) {
  ...
  return (
    <Card className="w-full py-3 rounded-none border-none shadow-none cursor-pointer" onClick={() => router.push(linkUrl)}>
      <CardContent className="flex flex-col items-start justify-start gap-1 p-0">
        ...
        <CardDescription className="text-sm text-muted-foreground line-clamp-1">{createdBy}</CardDescription>
      </CardContent>
    </Card>
  )
}